...
describe('<CardItem /> Component', () => {
	...
	test('should select one available size', () => {
		wrapper = mount(shallow(<CardItem store={store} product={mock} />).get(0));
	...
	test.skip('should show error when try to add to cart without select one size', () => {
		wrapper = mount(<CardItem store={store} product={mock} />);
		wrapper.find('.button__AddToCart').simulate('click');
